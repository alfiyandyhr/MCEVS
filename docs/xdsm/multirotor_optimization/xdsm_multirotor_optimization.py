...
x.add_input("opt",
			[r"W_\text{takeoff}^\text{init}, v_\text{cruise}^\text{init}",
			r"r_\text{rotor}^\text{init}, RPM_\text{rotor}^\text{init}"])
...
x.connect("D3", "opt", [r"G: \mu_\text{rotor}, CT/\sigma,", r"\mathbf{T/P_\text{segments}}"])
x.connect("D4", "opt", r"F: E_\text{total}")

x.add_output("opt", [r"W_\text{takeoff}^{*}, v_\text{cruise}^{*}", r"r_\text{rotor}^{*}, RPM_\text{rotor}^{*}"], side=LEFT)
x.add_output("D3", [r"G^{*}: \mu_\text{rotor}^{*}, CT/\sigma^{*},", r"\mathbf{T/P_\text{segments}^{*}}"])
