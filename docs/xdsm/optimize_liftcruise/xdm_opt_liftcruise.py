...
x.add_input("opt", r"v_{cruise}^{init}, S_{wing}^{init}, AR_{wing}^{init}, r_{rotor}^{init}, r_{prop}^{init}, J_{prop}^{init}")
x.add_input("D1", r"\rho_{air}, g")
x.add_input("D3", r"\rho_{air}, g, v_{climb}, N_{rotor}, FM")
x.add_input("D4", r"AoA_{cruise}, N_{prop}, \sigma_{prop}")
...
x.add_input("D8", r"N_{rotor}, N_{prop}, N_{blade}, tf")
...
x.connect("opt", "D1", r"S_{wing}, AR_{wing}, v_{cruise}")
x.connect("D1", "opt", r"CL_{cruise}")
x.connect("D1", "D2", r"Drag")
x.connect("D2", "D4", r"Thrust")
x.connect("opt", "D4", r"r_{prop}, J_{prop}, v_{cruise}")
x.connect("D4", "D6", r"P_{cruise}")
x.connect("D4", "opt", r"DL_{cruise}, C_{T}/\sigma_{prop}")
x.connect("solver", "D4", r"W_{takeoff}")
...
x.add_output("opt", "v_{cruise}^*, S_{wing}^*, AR_{wing}^*, r_{rotor}, r_{prop}, J_{prop}", side=LEFT)
x.add_output("solver", "W_{takeoff}^*", side=LEFT)
x.add_output("D3", "DL_{climb}^*", side=LEFT)
x.add_output("D1", "CL_{cruise}^*", side=LEFT)
x.add_output("D4", "DL_{cruise}^*, C_{T}/\sigma_{prop}^*", side=LEFT)
x.add_output("D5", "DL_{descent}^*", side=LEFT)
x.add_process(["opt", "D1", "D2", "D3", "D4", "D5", "D6", "D7", "D8", "D9", "D10", "D11", "solver", "opt"])
x.write("liftcruise_opt")