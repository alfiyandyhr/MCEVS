...
if run_with_speed_as_design_var_all_opt:
	...
	for i, mission_range in enumerate(range_array):
		...
		if battery_energy_density == 250:
			...
			if mission_range in [170]: mtow_guess = 900.0
		...
		with warnings.catch_warnings():
			warnings.simplefilter("ignore")
			results = RunStandardSingleObjectiveOptimization(vehicle, mission_ij, solution_fidelity, 'mission_time', mtow_guess, speed_as_design_var=True, print=False)
			# print(results)
