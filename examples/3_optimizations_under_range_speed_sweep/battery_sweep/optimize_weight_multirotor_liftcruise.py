...
run_battery_sweep_multirotor = True
...
if run_battery_sweep_multirotor:
    ...
    data = pd.read_csv('../selection_by_weight/multirotor/battery_250_Whpkg/results_without_speed_as_design_var.csv')
    ...
    for i, battery_energy_density in enumerate(battery_energy_density_list):
        ...
        with warnings.catch_warnings():
            ...
            results.update(RunStandardSingleObjectiveOptimization(vehicle, mission, fidelity, 'takeoff_weight', mtow_guess, speed_as_design_var=False, print=False))

        results_df = pd.DataFrame(results, index=[i])

        results_df.to_csv('weight_battery_sweep_multirotor.csv', mode='a', header=True if i == 0 else False)
...
if run_battery_sweep_liftcruise:
    ...
    for i, battery_energy_density in enumerate(battery_energy_density_list):
        ...
        results_df.to_csv('weight_battery_sweep_liftcruise.csv', mode='a', header=True if i == 0 else False)
