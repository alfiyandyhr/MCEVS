...
if plot_objective_all or plot_objective_separately or plot_constraint_separately:
	...
	opt_results_list = []
	opt_results_list.append(pd.read_csv(f'selection_by_weight/multirotor/optimization_results.csv'))
	opt_results_list.append(pd.read_csv(f'selection_by_energy/multirotor/optimization_results.csv'))
	opt_results_list.append(pd.read_csv(f'selection_by_weight/liftcruise/optimization_results.csv'))
	opt_results_list.append(pd.read_csv(f'selection_by_energy/liftcruise/optimization_results.csv'))
	test_results_list = []
	test_results_list.append(pd.read_csv(f'selection_by_weight/multirotor/test_results.csv'))
	test_results_list.append(pd.read_csv(f'selection_by_energy/multirotor/test_results.csv'))
	test_results_list.append(pd.read_csv(f'selection_by_weight/liftcruise/test_results.csv'))
	test_results_list.append(pd.read_csv(f'selection_by_energy/liftcruise/test_results.csv'))

	metrics = ['Weight|takeoff','Energy|entire_mission']
	...
	if plot_objective_all:
		...
		for i,metric in enumerate(metrics):
			for j,config in enumerate(configs):
				...
				axes[i,j].plot(opt_results['battery_energy_density'], opt_results[metric], 'k--', label=f'Optimized at test battery GED' if i==0 and j==0 else None)
		...
		fig.legend(loc='upper center', bbox_to_anchor=(0.505, 0.95), ncols=3)
	...
	if plot_objective_separately:
		...
		for i,metric in enumerate(metrics):
			for j,config in enumerate(configs):
				...
				plt.plot(opt_results['battery_energy_density'], opt_results[metric], 'k--', label=f'Optimized at test battery GED')
				plt.xlabel(r'Test battery GED $[Wh/kg]$')
				if metric == 'Weight|takeoff':
		...
		for i,metric in enumerate(metrics):
			for j,config in enumerate(configs):
				...
				if 2*i+j in [0,2]:
					...
					ylabel_names = [r'Blade loading $CT/\sigma$', r'Advance ratio $\mu$',r'Residual $[kg]$',r'$T/P$ HoverClimb $[g/W]$',r'$T/P$ Cruise $[g/W]$',r'$T/P$ HoverDescent $[g/W]$']
					...
					for l, ylabel in enumerate(ylabels):
						for k, selected_battery_energy_density in enumerate(selected_battery_energy_density_list):
							...
							if ylabel == 'LiftRotor|Cruise|CT/sigma': axes[l].hlines(0.14,xmin=xmin,xmax=xmax,linestyles='--',color='k',label='Constraint upper limit' if k==4 else None)
							...
							if l in [3,4,5]: axes[l].set_xlabel('Battery GED [Wh/kg]')
				...
				if 2*i+j in [1,3]:
					...
					ylabel_names = [r'Lift coeff $C_{L}$',r'Prop blade loading $CT/\sigma$',r'Prop advance ratio $J$',r'Residual $[kg]$',
									r'$T/P$ HoverClimb $[g/W]$',r'$T/P$ Cruise $[g/W]$',r'$T/P$ HoverDescent $[g/W]$',r'Clearance constraint $[m]$']

					fig, axes = plt.subplots(2, 4, sharex=True, figsize=(16,7))
					axes = axes.flatten()
					...
					for l, ylabel in enumerate(ylabels):
						for k, selected_battery_energy_density in enumerate(selected_battery_energy_density_list):
							...
							if ylabel == 'Propeller|Cruise|CT/sigma': axes[l].hlines(0.14,xmin=xmin,xmax=xmax,linestyles='--',color='k',label='Constraint upper limit' if k==4 else None)
							...
							if l in [4,5,6,7]: axes[l].set_xlabel('Battery GED [Wh/kg]')
							axes[l].set_ylabel(ylabel_names[l])
...
if plot_compare_optimals:
	opt_results11 = pd.read_csv(f'selection_by_weight/multirotor/optimization_results.csv')
	opt_results12 = pd.read_csv(f'selection_by_weight/liftcruise/optimization_results.csv')
	opt_results21 = pd.read_csv(f'selection_by_energy/multirotor/optimization_results.csv')
	opt_results22 = pd.read_csv(f'selection_by_energy/liftcruise/optimization_results.csv')
	...
	axes[0].set_ylabel(r'Takeoff weight $[kg]$')
