	...
	if plot_objective_separately:
		...
		for i,metric in enumerate(metrics):
			for j,config in enumerate(configs):
				...
				plt.plot(opt_results['battery_energy_density'], opt_results['Weight|takeoff' if metric == 'Weight|gross_takeoff' or metric == 'Weight|takeoff' else 'Energy|entire_mission'], 'k--', label=f'Optimized at test battery GED')
