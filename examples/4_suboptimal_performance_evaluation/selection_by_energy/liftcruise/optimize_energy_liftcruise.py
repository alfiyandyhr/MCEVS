...
# selected_battery_energy_density_list = [250,550,850]
...
if do_optimizaton:
	...
	for i, battery_energy_density in enumerate(battery_energy_density_list):
		...
		if battery_energy_density in [475,525,725,750]:
		...
		with warnings.catch_warnings():
			...
			results_df.to_csv(f'optimization_results_.csv', mode='a', header=True if i==0 else False)
...
if do_test:
	...
	for i, selected_battery_energy_density in enumerate(selected_battery_energy_density_list):
		...
		for j, battery_energy_density in  enumerate(battery_energy_density_list):
			...
			res = analysis.evaluate(record=False, mtow_guess=2500.0, print=False)
...
if do_plot_constraints:
	...
	for j, ylabel in enumerate(ylabels):
		for i, selected_battery_energy_density in enumerate(selected_battery_energy_density_list):
			...
			if ylabel == 'Propeller|Cruise|CT/sigma': axes[j].hlines(0.14,xmin=xmin,xmax=xmax,linestyles='--',color='k',label='Constraint upper limit' if i==4 else None)
			...
			if ylabel == 'LiftRotor|clearance_constraint': axes[j].hlines(0.1,xmin=xmin,xmax=xmax,linestyles='--',color='k')

			if j in [4,5,6,7]: axes[j].set_xlabel('Battery GED [Wh/kg]')
