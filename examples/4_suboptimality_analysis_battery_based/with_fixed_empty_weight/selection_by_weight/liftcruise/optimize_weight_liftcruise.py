...
if do_optimizaton:
    ...
    for i, battery_energy_density in enumerate(battery_energy_density_list):
        ...
        with warnings.catch_warnings():
            ...
            results_df.to_csv('optimization_results.csv', mode='a', header=True if i == 0 else False)
...
if do_test:
    ...
    for i, selected_battery_energy_density in enumerate(selected_battery_energy_density_list):
        ...
        for j, battery_energy_density in enumerate(battery_energy_density_list):
            ...
            vehicle.battery.density = float(selected_battery_energy_density)
            ...
            # MTOW Analysis
            mtow_analysis = WeightAnalysis(vehicle, mission, solution_fidelity, weight_type='maximum', sizing_mode=True, solved_by='optimization')
            mtow_analysis.evaluate(record=False, weight_guess=2500.0, print=False)
            ...
            results_df.to_csv('test_results.csv', mode='a', header=True if i * 3 + j == 0 else False)
...
if do_plot_objective:
    ...
    opt_results.loc[opt_results['Weight|residual'] > 0.1, 'Weight|takeoff'] = np.nan
    opt_results.loc[opt_results['Aero|Cruise|CL'] > 0.91, 'Weight|takeoff'] = np.nan
    opt_results.loc[opt_results['LiftRotor|HoverClimb|T_to_P'] > 12.01, 'Weight|takeoff'] = np.nan
    opt_results.loc[opt_results['Propeller|Cruise|T_to_P'] > 12.01, 'Weight|takeoff'] = np.nan
    opt_results.loc[opt_results['LiftRotor|HoverDescent|T_to_P'] > 12.01, 'Weight|takeoff'] = np.nan
    opt_results.loc[opt_results['Propeller|Cruise|J'] > 3.01, 'Weight|takeoff'] = np.nan
    opt_results.loc[opt_results['Propeller|Cruise|CT/sigma'] > 0.141, 'Weight|takeoff'] = np.nan
    opt_results.loc[opt_results['LiftRotor|clearance_constraint'] > 0.1, 'Weight|takeoff'] = np.nan
    ...
    test_results[ylabel] = np.where(condition1 & condition2, np.nan, test_results[ylabel])
