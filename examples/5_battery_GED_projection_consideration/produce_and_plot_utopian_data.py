	...
	for i, scenario in enumerate(battery_dict):
		...
		for j, year in enumerate(year_list):
			...
			with warnings.catch_warnings():
				...
				results_df.to_csv(f'optimal_results_by_scenario_by_year/weight_opt_multirotor_results.csv', mode='a', header=True if k==0 else False)
	...
	for i, scenario in enumerate(battery_dict):
		...
		for j, year in enumerate(year_list):
			...
			with warnings.catch_warnings():
				...
				results.update(RunStandardSingleObjectiveOptimization(vehicle, mission, solution_fidelity, 'takeoff_weight', mtow_guess, speed_as_design_var=False, print=True))
				results_df = pd.DataFrame(results, index=[k])
				results_df.to_csv(f'optimal_results_by_scenario_by_year/weight_opt_liftcruise_results.csv', mode='a', header=True if k==0 else False)
	...
	for i, scenario in enumerate(battery_dict):
		...
		for j, year in enumerate(year_list):
			...
			with warnings.catch_warnings():
				...
				results_df = pd.DataFrame(results, index=[k])
				results_df.to_csv(f'optimal_results_by_scenario_by_year/energy_opt_multirotor_results.csv', mode='a', header=True if k==0 else False)
	...
	for i, scenario in enumerate(battery_dict):
		...
		for j, year in enumerate(year_list):
			...
			with warnings.catch_warnings():
				...
				results.update(RunStandardSingleObjectiveOptimization(vehicle, mission, solution_fidelity, 'energy', mtow_guess, speed_as_design_var=False, print=True))
				results_df = pd.DataFrame(results, index=[k])
				results_df.to_csv(f'optimal_results_by_scenario_by_year/energy_opt_liftcruise_results.csv', mode='a', header=True if k==0 else False)
...
if plot_utopian_data:
	...
	for i, metric in enumerate(metrics):
		for j, config in enumerate(configs):
			...
			if i == 1: axes[i,j].set_xlabel('Year')
