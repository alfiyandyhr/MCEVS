    ...
    if plot_objective_all:
        ...
        for i, metric in enumerate(metrics):
            for j, config in enumerate(configs):
                ...
                for l, coeff_type in enumerate(weight_coeffs_options):  # noqa: E741
    ...
    if plot_objective_separately:
        ...
        for i, metric in enumerate(metrics):
            for j, config in enumerate(configs):
                ...
                plt.plot(opt_results['battery_energy_density'], opt_results['Weight|takeoff' if metric == 'Weight|gross_takeoff' or metric == 'Weight|takeoff' else 'Energy|entire_mission'], 'k--', ms=4.0, label='Optimized at test battery GED')
                    ...
                    for l, ylabel in enumerate(ylabels):  # noqa: E741
                        for k, selected_battery_energy_density in enumerate(selected_battery_energy_density_list):
                            ...
                            if ylabel == 'Propeller|Cruise|CT/sigma':
                                axes[l].hlines(0.14, xmin=xmin, xmax=xmax, linestyles='--', color='k', label='Constraint upper limit' if k == 1 else None)
