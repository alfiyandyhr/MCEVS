...
if do_optimizaton:
    ...
    for i, coeff_type in enumerate(weight_coeffs_options):
        ...
        if coeff_type == 'uniform':
            mtow_guess_list = [1200.0, 1200.0, 1200.0, 1200.0]
...
if do_test:
    ...
    opt_results = pd.read_csv('../../../../4_suboptimal_performance_evaluation/with_fixed_empty_weight/selection_by_energy/liftcruise/optimization_results.csv')
    ...
    for i, coeff_type in enumerate(weight_coeffs_options):
        ...
        for j, battery_energy_density in enumerate(battery_energy_density_list):
            ...
            wing_area = multipoint_opt_results[multipoint_opt_results['coeff_type'] == coeff_type]['Wing|area'].to_numpy()[0]
            wing_aspect_ratio = multipoint_opt_results[multipoint_opt_results['coeff_type'] == coeff_type]['Wing|aspect_ratio'].to_numpy()[0]
            r_lift_rotor = multipoint_opt_results[multipoint_opt_results['coeff_type'] == coeff_type]['LiftRotor|radius'].to_numpy()[0]
            r_propeller = multipoint_opt_results[multipoint_opt_results['coeff_type'] == coeff_type]['Propeller|radius'].to_numpy()[0]
            RPM_propeller_cruise = multipoint_opt_results[multipoint_opt_results['coeff_type'] == coeff_type]['Propeller|Cruise|RPM'].to_numpy()[0]
            ...
            f_non_hub_non_wing = opt_results[opt_results['battery_energy_density'] == 250]['Aero|Cruise|f_total_non_hub_non_wing'].to_numpy()[0]
            Cd0_wing = opt_results[opt_results['battery_energy_density'] == 250]['Aero|Cruise|Cd0_wing'].to_numpy()[0]
...
if do_plot_objective:
    ...
    opt_results = pd.read_csv('../../../../4_suboptimal_performance_evaluation/with_fixed_empty_weight/selection_by_energy/liftcruise/optimization_results.csv')
    test_results = pd.read_csv('../../../../4_suboptimal_performance_evaluation/with_fixed_empty_weight/selection_by_energy/liftcruise/test_results.csv')
    ...
    test_results[ylabel] = np.where((test_results['Weight|max_takeoff'] < test_results['Weight|gross_takeoff']) & (test_results['battery_energy_density_opt'] != test_results['battery_energy_density_test']), np.nan, test_results[ylabel])
    ...
    multipoint_test_results[ylabel] = np.where((multipoint_test_results['Weight|max_takeoff'] < multipoint_test_results['Weight|gross_takeoff']) & (multipoint_test_results['battery_energy_density_test'] != 250), np.nan, multipoint_test_results[ylabel])
    multipoint_test_results.loc[multipoint_test_results['Weight|residual'] > 0.1, ylabel] = np.nan
    multipoint_test_results.loc[multipoint_test_results['Aero|Cruise|CL'] > 0.91, ylabel] = np.nan
    multipoint_test_results.loc[multipoint_test_results['LiftRotor|HoverClimb|T_to_P'] > 12.01, ylabel] = np.nan
    multipoint_test_results.loc[multipoint_test_results['Propeller|Cruise|T_to_P'] > 12.01, ylabel] = np.nan
    multipoint_test_results.loc[multipoint_test_results['LiftRotor|HoverDescent|T_to_P'] > 12.01, ylabel] = np.nan
    multipoint_test_results.loc[multipoint_test_results['Propeller|Cruise|J'] > 3.01, ylabel] = np.nan
    multipoint_test_results.loc[multipoint_test_results['Propeller|Cruise|CT/sigma'] > 0.141, ylabel] = np.nan
    multipoint_test_results.loc[multipoint_test_results['LiftRotor|clearance_constraint'] > 0.1, ylabel] = np.nan
    ...
    plt.plot(opt_results['battery_energy_density'], opt_results[ylabel], 'k--', label='Optimized at test battery GED')
    plt.title('Suboptimal performance evaluation\nfor Lift+Cruise from energy-based optimization')
    ...
    for j, ylabel in enumerate(ylabels):
        for i, selected_battery_energy_density in enumerate(selected_battery_energy_density_list):
            ...
            if ylabel == 'Aero|Cruise|CL':
                axes[j].hlines(0.9, xmin=xmin, xmax=xmax, linestyles='--', color='k')
            if ylabel == 'Propeller|Cruise|CT/sigma':
                axes[j].hlines(0.14, xmin=xmin, xmax=xmax, linestyles='--', color='k', label='Constraint upper limit' if i == 0 else None)
            if ylabel == 'Propeller|Cruise|J':
                axes[j].hlines(3.001, xmin=xmin, xmax=xmax, linestyles='--', color='k')
            ...
            if ylabel == 'LiftRotor|clearance_constraint':
                axes[j].hlines(1e-5, xmin=xmin, xmax=xmax, linestyles='--', color='k')
        ...
        for k, coeff_type in enumerate(weight_coeffs_options):
            if coeff_type == 'uniform':
                ...
                axes[j].plot(battery_energy_density_list, multipoint_test_results_k, 'o-', ms=3.0, label=f'Multipoint {coeff_type}' if j == 1 else None)
            if coeff_type == 'linear':
                multipoint_test_results_k = multipoint_test_results[multipoint_test_results['coeff_type'] == coeff_type][ylabel]
                axes[j].plot(battery_energy_density_list, multipoint_test_results_k, 'x-', label=f'Multipoint {coeff_type}' if j == 1 else None)
