...
if do_optimizaton:
	...
	for i, coeff_type in enumerate(weight_coeffs_options):
		...
		with warnings.catch_warnings():
			...
			results_df.to_csv(f'multipoint_opt_results.csv', mode='a', header=True if i==0 else False)
...
if do_test:
	...
	for i, coeff_type in enumerate(weight_coeffs_options):
		...
		for j, battery_energy_density in  enumerate(battery_energy_density_list):
			...
			results_df.to_csv(f'multipoint_test_results.csv', mode='a', header=True if i*2+j==0 else False)
...
if do_plot_constraints:
	...
	for j, ylabel in enumerate(ylabels):
		for i, selected_battery_energy_density in enumerate(selected_battery_energy_density_list):
			...
			if ylabel == 'Propeller|Cruise|CT/sigma': axes[j].hlines(0.14,xmin=xmin,xmax=xmax,linestyles='--',color='k',label='Constraint upper limit' if i==0 else None)
