...
if do_test:
	...
	opt_results = pd.read_csv('../../../../4_suboptimal_performance_evaluation/with_fixed_empty_weight/selection_by_energy/multirotor/optimization_results.csv')
...
if do_plot_objective:
	opt_results = pd.read_csv('../../../../4_suboptimal_performance_evaluation/with_fixed_empty_weight/selection_by_energy/multirotor/optimization_results.csv')
	test_results = pd.read_csv('../../../../4_suboptimal_performance_evaluation/with_fixed_empty_weight/selection_by_energy/multirotor/test_results.csv')
	...
	plt.plot(opt_results['battery_energy_density'], opt_results[ylabel], 'k--', ms=4.0, label=f'Optimized at test battery GED')
	plt.title('Suboptimal performance evaluation\nfor Multirotor from energy-based optimization')
...
if do_plot_constraints:
	...
	for j, ylabel in enumerate(ylabels):
		for i, selected_battery_energy_density in enumerate(selected_battery_energy_density_list):
			...
			if ylabel == 'LiftRotor|Cruise|CT/sigma': axes[j].hlines(0.14,xmin=xmin,xmax=xmax,linestyles='--',color='k',label='Constraint upper limit' if i==2 else None)
		...
		for k, coeff_type in enumerate(weight_coeffs_options):
			if coeff_type == 'uniform':
				...
				axes[j].plot(battery_energy_density_list, multipoint_test_results_k, 'o-', ms=3.0, label=f'Multipoint {coeff_type}' if j==0 else None)
			if coeff_type == 'linear':
				multipoint_test_results_k = multipoint_test_results[multipoint_test_results['coeff_type']==coeff_type][ylabel]
				axes[j].plot(battery_energy_density_list, multipoint_test_results_k, 'x-', label=f'Multipoint {coeff_type}' if j==0 else None)	
