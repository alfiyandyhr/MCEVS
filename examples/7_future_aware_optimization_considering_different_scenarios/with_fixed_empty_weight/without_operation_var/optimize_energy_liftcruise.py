...
from MCEVS.Optimization.Standard import RunStandardSingleObjectiveOptimization, RunMultiPointSingleObjectiveOptimization
	...
	for i, scenario in enumerate(battery_dict):
		...
		for j, year1 in enumerate(year_list):
			...
			for k, year2 in enumerate(year_list):
				...
				vehicle.battery.density = float(battery_energy_density_list[0])
...
if plot_suboptimal_data_from_utopian_2D:

	parameter = 'Energy|entire_mission'
	# parameter = 'Weight|gross_takeoff'
	...
	for i, scenario in enumerate(battery_dict):
		...
		data = opt_test_results[opt_test_results['scenario']==scenario]
		utopian_data_i = utopian_data[utopian_data['scenario']==scenario]

		for j, year_opt in enumerate(year_list):
			data_j = data[data['year_opt']==year_opt]

			# Plot data on each subplot
			axes[i].plot(data_j['year_test'], data_j[parameter], '-', markersize=4)
			axes[i].set_title(f'{scenario}')
			axes[i].set_xlabel('Year')
			if i==0: axes[i].set_ylabel(f'{parameter}')
...
if plot_suboptimal_data_from_utopian_contour:
	...
	data_df = pd.read_csv('opt_test_results.csv')
	...
	for i, scenario in enumerate(battery_dict):
		...
		data_df_i = data_df[data_df['scenario']==scenario]

		X, Y = np.meshgrid(year_list, year_list)
		Z = data_df_i[parameter].to_numpy().reshape(len(year_list), len(year_list)).T
		...
		if i == 0: ax.set_ylabel('Tested Year')
		...
		cbar = fig.colorbar(cp, ax=ax, orientation='horizontal', fraction=0.05, pad=0.13)
