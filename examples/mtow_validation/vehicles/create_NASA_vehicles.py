...
if create_vehicle1:
	...
	vehicle1.print_info()
	create_NASA_QuadRotor_vsp3('NASA_QuadRotor.vsp3', vehicle1)
...
if create_vehicle2:
	...
	vehicle2.add_component(kind='propeller', n_propeller=1, n_blade=3, n_section=8, airfoil='BOEING_VERTOL_VR12', radius=r_propeller, hub_radius=0.21*r_propeller, chord=c_propeller, r_to_R_list=r_to_R_list2, c_to_R_list=c_to_R_list2, pitch_list=pitch_list2, hub_length=0.3048, hub_max_diameter=1.5*0.3048, figure_of_merit=0.75, technology_factor=0.8)

	vehicle2.add_component(kind='boom', represented_as='fuselage', length=16.0*0.3048, max_diameter=0.3048, number_of_booms=4)
	vehicle2.print_info()
	create_NASA_LiftPlusCruise_vsp3('NASA_LiftPlusCruise.vsp3', vehicle2)
