...
if run_without_speed_as_design_var_one_opt or run_with_speed_as_design_var_one_opt:
	...
	# mission_ij = StandardMissionProfile(90000, 72*1000/3600)
...
if run_without_speed_as_design_var_all_opt:
	...
	for i, mission_range in enumerate(range_array):
		for j, cruise_speed in enumerate(speed_array):
			...
			# if mission_range in [10] and cruise_speed in [180]:
			if cruise_speed == 252:
				...
				# results_df.to_csv('Multirotor_OPT_results_without_speed_as_design_var.csv', mode='a', header=True if iter_idx==1 else False)
				results_df.to_csv('Multirotor_OPT_results_test.csv', mode='a', header=True if iter_idx==7 else False)
...
if run_with_speed_as_design_var_all_opt:
	...
	for i, mission_range in enumerate(range_array):
		...
		results_df.to_csv('Multirotor_OPT_results_with_speed_as_design_var.csv', mode='a', header=True if i==0 else False)
