...
if rerun_without_speed_as_design_var_all_opt:
	...
	for i, mission_range in enumerate(range_array):
		for j, cruise_speed in enumerate(speed_array):
			...
			if mission_range in [10] and cruise_speed == 150:
				iter_idx = i*len(speed_array)+j+1
				print(f"Iter= {iter_idx}, Range= {mission_range}, Speed= {cruise_speed}")
				sys.stdout.flush() # To flush the above print output
				...
				# Different initial guesses
				if battery_energy_density == 250:
					if mission_range in [140] and cruise_speed == 150: mtow_guess = 2500.0
					if mission_range in [160] and cruise_speed == 160: mtow_guess = 2500.0
					if mission_range in [180] and cruise_speed in [170,180]: mtow_guess = 2500.0
					if mission_range in [190] and cruise_speed == 320: mtow_guess = 2000.0
				elif battery_energy_density == 400:
					pass
				elif battery_energy_density == 550:
					if mission_range in [10] and cruise_speed == 150: mtow_guess = 2500.0
