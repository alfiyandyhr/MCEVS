...
if run_with_speed_as_design_var_all_opt:
	...
	for i, mission_range in enumerate(range_array):
		...
		cruise_speed_guess = 320 # km/h

		if battery_energy_density == 250:
			if mission_range in [60]: mtow_guess = 2000.0
			elif mission_range in [70,80,90]: mtow_guess = 2500.0
			elif mission_range in [120]: mtow_guess = 5000.0
			elif mission_range in [130]: mtow_guess = 3400.0
			else: mtow_guess = 1000.0
		elif battery_energy_density == 400:
			if mission_range in [40]: mtow_guess = 500.0
			else: mtow_guess = 600.0
