...
class BEMTSolver:
    ...
    def __init__(self, rotorDict: dict, sectionDict: dict, fluidDict: dict):
        ...
        if all(key in rotorDict for key in rotorDict_keys):
            pass
        else:
            raise SyntaxError('Keys in rotorDict are not complete.')

        if all(key in sectionDict for key in sectionDict_keys):
            pass
        else:
            raise SyntaxError('Keys in sectionDict are not complete.')

        if all(key in fluidDict for key in fluidDict_keys):
            pass
        else:
            raise SyntaxError('Keys in fluidDict are not complete.')
