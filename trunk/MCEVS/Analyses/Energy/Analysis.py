...
class EnergyAnalysis(object):
	...
	def __init__(self, vehicle:object, mission:object, fidelity:dict):
		...
		for drag_type, aero_model in self.fidelity['aerodynamics'].items():
		...
		for segment, power_model in self.fidelity['power_model'].items():
		...
		for component, weight_model in self.fidelity['weight_model'].items():
