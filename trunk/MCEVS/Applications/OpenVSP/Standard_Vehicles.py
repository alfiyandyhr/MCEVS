...
def create_NASA_LiftPlusCruise_vsp3(fname:str, vehicle:object):
	...
	rotor_hub_ids, rotor_ids = NASA_LPC_Lift_Rotor(n_lift_rotor=n_lift_rotor, n_blade=n_blade_rotor, r_lift_rotor=r_lift_rotor, l_fuse=l_fuse, wing_S=wing_S, wing_AR=wing_AR, boom_ids=boom_ids)
	prop_hub_id, prop_id = NASA_LPC_Propeller(n_propeller=n_propeller, n_blade=n_blade_prop, r_propeller=r_propeller, l_fuse=l_fuse, fuse_id=fuse_id)
	vsp.WriteVSPFile(fname)
	vsp.ClearVSPModel()