...
class CruiseConstantSpeed():
	"""
	docstring for CruiseConstantSpeed
	kwargs:
		speed 		: constant hover climb speed [m/s]
		distance 	: distance of the hover climb [m]
		duration	: duration of the hover climb [s]
	"""
	def __init__(self, id:int, name:str, kwargs:dict, n_discrete=5):
		super(CruiseConstantSpeed, self).__init__()
	...
	def _calc_position(self, x_list, y_list, t_next):
		...
		x_next = x0 + self.speed * (t_next - t0)
	...
	def _calc_velocity(self, vx_list, vy_list, t_next):
		vx0, vy0, t0 = self.speed, 0.0, t_next[0]
