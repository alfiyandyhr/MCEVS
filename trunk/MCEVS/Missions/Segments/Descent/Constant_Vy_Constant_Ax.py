...
class DescentConstantVyConstantAx():
	"""
	docstring for DescentConstantVyConstantAx
	kwargs:
		...
		deceleration_X 	: constant deceleration in X-direction [m/s**2]
	...
	def __init__(self, id:int, name:str, initial_speed_X:float, kwargs:dict, n_discrete=10):
		super(DescentConstantVyConstantAx, self).__init__()
		self.id = id 								# segment id
		self.name = name 							# segment name
		self.kind = 'DescentConstantVyConstantAx'	# segment kind
	...
	def _initialize(self):
		for item in list(self.kwargs):
			...
			elif item == 'deceleration_X':
				self.deceleration_X = float(self.kwargs[item])
		...
		self.deceleration_X = (self.initial_speed_X - self.final_speed_X) / self.duration
		self.distance_X = self.initial_speed_X * self.duration - 0.5 * self.deceleration_X * self.duration**2
	...
	def _calc_position(self, x_list, y_list, t_next):
		...
		x_next = x0 + self.initial_speed_X * (t_next - t0) - 0.5 * self.deceleration_X * (t_next - t0)**2
	...
	def _calc_velocity(self, vx_list, vy_list, t_next):
		...
		# Velocity under constant deceleration in x
		vx_next = vx0 - self.deceleration_X * (t_next - t0)
	...
	def _calc_acceleration(self, ax_list, ay_list, t_next):
		# Zero deceleration in y, and constant deceleration in x
		ax_next = - self.deceleration_X * np.ones_like(t_next)
	...
	def _info(self):
		...
		info += f'\t\t\tdeceleration_X = {self.deceleration_X} m/s2\n'
	...
	def print_info(self):
		...
		print(f'\tdeceleration_X = {self.deceleration_X} m/s2')
