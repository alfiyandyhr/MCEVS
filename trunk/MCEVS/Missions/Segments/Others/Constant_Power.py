...
class ConstantPower():
	"""
	docstring for ConstantPower
	kwargs:
		duration			: duration of the taxi [s]
		percent_max_power 	: power percentage from max power (typically hover (climb) power)
	"""
	def __init__(self, id:int, name:str, kwargs:dict, n_discrete=10):
		super(ConstantPower, self).__init__()
		self.id = id 					# segment id
		self.name = name 				# segment name
		self.kind = 'ConstantPower'		# segment kind
		self.n_discrete = n_discrete	# mission discretization
		self.kwargs = kwargs
		self.duration = None
		self.percent_max_power = None

	def _initialize(self):
		for item in list(self.kwargs):
			if item == 'duration':
				self.duration = float(self.kwargs[item])
			elif item == 'percent_max_power':
				self.percent_max_power = float(self.kwargs[item])

		if self.duration is None:
			raise NameError("Need to define the duration of hover.")

		if self.percent_max_power is None:
			raise NameError("Need to define the power percentage from maximum power.")
	...
	def _info(self):
		...
		info += f'\t\t\tPercent max power = {self.percent_max_power}%'
		return info
	...
	def print_info(self):
		...
		print(f'\tPercent max power = {self.percent_max_power} %')
