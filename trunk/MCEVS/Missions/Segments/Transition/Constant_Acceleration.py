...
class TransitionConstantAcceleration():
	"""
	docstring for TransitionConstantAcceleration in x-direction
	kwargs:
		final_speed		: final transition speed [m/s]
		acceleration 	: constant transition acceleration [m/s**2]
		distance 		: distance of the transition [m]
		duration		: duration of the transition [s]
	"""
	def __init__(self, id:int, name:str, initial_speed:float, kwargs:dict, n_discrete=5):
		super(TransitionConstantAcceleration, self).__init__()
		self.id = id 									# segment id
		self.name = name 								# segment name
		self.kind = 'TransitionConstantAcceleration'	# segment kind
	...
	def _calc_position(self, x_list, y_list, t_next):
		...
		x_next = x0 + self.initial_speed * (t_next - t0) + 0.5 * self.acceleration * (t_next - t0)**2
		y_next = y0 + np.zeros_like(t_next)
		x_list.append(x_next)
		y_list.append(y_next)
		return x_list, y_list

	def _calc_velocity(self, vx_list, vy_list, t_next):
		vx0, vy0, t0 = vx_list[-1][-1], 0.0, t_next[0]
		# Velocity under constant acceleration in x
		vx_next = vx0 + self.acceleration * (t_next - t0)
		vy_next = vy0 + np.zeros_like(t_next)
		vx_list.append(vx_next)
		vy_list.append(vy_next)
		return vx_list, vy_list

	def _calc_acceleration(self, ax_list, ay_list, t_next):
		# Zero acceleration in y, constant acceleration in x
		ax_next = self.acceleration * np.ones_like(t_next)
