...
def plot_geometries(vehicle_list:list, label_list:list, cruise_speed_list=[None],
	...
	if with_operation_var: cruise_speed_list = np.round(np.array(cruise_speed_list),1)
	...
	if vehicle_list[0].configuration == 'Multirotor':
		...
		if with_operation_var:
			...
			for k, cruise_speed in enumerate(cruise_speed_list):
				...
				if len(cruise_speed_list) == 1: arrow_loc_x = center_x
				elif len(cruise_speed_list) == 2: arrow_loc_x = center_x+(k-1/2)
				elif len(cruise_speed_list) == 3: arrow_loc_x = center_x+(k/2-1/2)
				elif len(cruise_speed_list) == 4: arrow_loc_x = center_x+(k/2-0.75)
		...
		if with_operation_var:
			...
			for k, num in enumerate(cruise_speed_list):
				...
				if k < len(cruise_speed_list) - 1:
					ax.text(start_x, start_y, ",", color="black", fontsize=12, ha="left", va="center")
					start_x += spacing
				if len(cruise_speed_list) == 1: start_x += 0.25
			ax.text(start_x, start_y, "]", color="black", fontsize=12, ha="left", va="center")
			...
			for k, num in enumerate(RPM_list):
				...
				if k < len(RPM_list) - 1:
					ax.text(start_x, start_y, ",", color="black", fontsize=12, ha="left", va="center")
					start_x += spacing
				if len(RPM_list) == 1: start_x += 0.25
	...
	if vehicle_list[0].configuration == 'LiftPlusCruise':
		...
		for i, vehicle in enumerate(vehicle_list):
			...
			if with_operation_var:
				...
				if len(cruise_speed_list) == 1: arrow_loc_x = center_x
				elif len(cruise_speed_list) == 2: arrow_loc_x = center_x+(i-1/2)
				elif len(cruise_speed_list) == 3: arrow_loc_x = center_x+(i/2-1/2)
