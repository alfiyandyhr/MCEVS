...
def StandardMultirotorEVTOL(design_var:dict, n_pax=4, mtow=None):
	...
	c_lift_rotor 	= design_var['c_lift_rotor']
	...
	if n_pax == 4:
		...
		vehicle.add_component(kind='lift_rotor', n_rotor=4, n_blade=5, radius=r_lift_rotor, chord=c_lift_rotor,figure_of_merit=0.75, advance_ratio=rotor_mu, technology_factor=0.8)
...
def StandardLiftPlusCruiseEVTOL(design_var:dict, n_pax=4, mtow=None):
	...
	c_propeller 	= design_var['c_propeller']
	...
	if n_pax == 4:
		...
		vehicle.add_component(kind='lift_rotor', n_rotor=8, n_blade=2, radius=r_lift_rotor, chord=c_lift_rotor, hub_length=l_rotor_hub, hub_max_diameter=d_rotor_hub, figure_of_merit=0.75, technology_factor=0.8)
		vehicle.add_component(kind='propeller', n_propeller=1, n_blade=4, radius=r_propeller, chord=c_propeller, advance_ratio=propeller_J, hub_length=l_prop_hub, hub_max_diameter=d_prop_hub, technology_factor=0.8)
