...
def StandardMultirotorEVTOL(design_var:dict, mtow=None):
	...
	vehicle.add_component(kind='lift_rotor', n_rotor=7, n_blade=4, solidity=0.13, radius=r_lift_rotor, technology_factor=0.8)
...
def StandardLiftPlusCruiseEVTOL(design_var:dict, mtow=None):
	...
	vehicle.add_component(kind='fuselage', length=6.7, max_diameter=1.8, number_of_passenger=4, technology_factor=0.8)
	vehicle.add_component(kind='landing_gear', strut_length=0.7, ultimate_load_factor=5.7, technology_factor=0.8)
	vehicle.add_component(kind='lift_rotor', n_rotor=6, n_blade=4, solidity=0.13, radius=r_lift_rotor, technology_factor=0.8)
	vehicle.add_component(kind='propeller', n_propeller=1, n_blade=4, solidity=0.13, radius=r_propeller, technology_factor=0.8)
